 ...
 */
export function parquetDataFrame(
  ...
  let groupStart = 0;
  const groupEnds = metadata.row_groups.map(
    (group) => (groupStart += Number(group.num_rows))
  );
  ...
  return {
    ...
    rows(rowStart: number, rowEnd: number, orderBy?: string) {
      if (orderBy) {
        ...
        const wrapped = new Array<ResolvableRow | null>(numRows)
          .fill(null)
          .map(() =>
            Object.fromEntries(header.map((key) => [key, resolvablePromise()]))
          );
        getSortIndex(orderBy)
          .then((indices) => {
